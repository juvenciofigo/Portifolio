  ...
  >
    <article>
      ...
      <time class="gap-2 font-mono text-sm font-semibold text-gray-700 sm:flex sm:text-sm">
        {{ getDate(props.startDate) }}  <span v-if="props.endDate">- {{ getDate(props.endDate) }}</span>
        <p v-if="props.startDate && props.endDate">({{ getTimeBetween(props.startDate, props.endDate) }})</p>
      </time>
      <span class="block">
        <component
          :is="props.url == '#' ? 'h2' : 'a'"
        ...
        >
          <span v-if="props.company && props.company !== ' '">@ {{ props.company }}</span>
          <svg
            v-if="url && url != '#'"
